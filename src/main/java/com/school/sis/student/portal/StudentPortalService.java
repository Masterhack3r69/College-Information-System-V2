...
public class StudentPortalService {
    ...
    @Transactional(readOnly=true) public List<Map<String,Object>> finance(SisUserDetails p){return jdbc.queryForList("""
      select a.id,a.total_assessment as "totalAssessment",a.amount_paid as "amountPaid",a.balance,a.status,sy.school_year as "schoolYear",sm.name as "semesterName"
    ...
    @Transactional(readOnly=true) public List<Map<String,Object>> payments(SisUserDetails p){return jdbc.queryForList("select ap.id,ap.official_receipt_number as \"receiptNumber\",ap.amount,ap.payment_method as \"paymentMethod\",ap.paid_at as \"paidAt\",ap.status,ap.void_reason as \"voidReason\" from assessment_payments ap where ap.student_id=? order by ap.paid_at desc",access.studentId(p));}

...
async function refreshAccessToken() {
  ...
  if (!refreshPromise) refreshPromise = fetch(`${baseUrl}/auth/refresh`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ refreshToken }) })
    .then(async r => {
      if (!r.ok) return null
...
export async function rawApi<T>(absolutePath: string, init: RequestInit = {}, retry = true): Promise<T> {
  const headers = new Headers(init.headers)
  headers.set("Content-Type", "application/json")
  if (accessToken) headers.set("Authorization", `Bearer ${accessToken}`)
  const response = await fetch(absolutePath, { ...init, headers })
  if (response.status === 401 && retry && await refreshAccessToken()) return rawApi<T>(absolutePath, init, false)
...
export async function openPdf(path: string) {
  ...
  if (response.status === 401 && await refreshAccessToken()) { headers.set("Authorization", `Bearer ${accessToken}`); response = await fetch(`${baseUrl}${path}`, { headers }) }

...
export default function StudentAcademicsPage() {
  ...
  return (
    <div className="mx-auto max-w-[1220px] p-5 md:p-8">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div><h1 className="flex items-center gap-3 text-3xl font-semibold text-[#092f66]"><BookOpen />Academics</h1><p className="mt-2 text-slate-600">Academic plan, posted grades, approved credits, evaluations, and graduation audits.</p></div>
      ...
      <Tabs defaultValue="plan" className="mt-7">
        ...
        <TabsContent value="plan"><div className="mt-5 overflow-hidden rounded-xl border"><Table><TableHeader><TableRow><TableHead>Course</TableHead><TableHead>Curriculum term</TableHead><TableHead>Requirement</TableHead><TableHead>Status</TableHead><TableHead>Detail</TableHead></TableRow></TableHeader><TableBody>
          {plan.data?.items.map((item) => <TableRow key={item.curriculumCourseId}><TableCell><p className="font-medium">{item.courseCode}</p><p className="text-xs text-slate-500">{item.courseTitle} · {item.creditUnits} units</p></TableCell><TableCell>Year {item.yearLevel} · {item.semester.replaceAll("_", " ")}</TableCell><TableCell>{item.requirement}</TableCell><TableCell><Badge variant={statusVariant(item.status)}>{item.status.replaceAll("_", " ")}</Badge></TableCell><TableCell className="max-w-64 text-sm text-slate-500">{item.detail ?? "—"}</TableCell></TableRow>)}
        ...
        <TabsContent value="grades"><div className="mt-5 overflow-hidden rounded-xl border"><Table><TableHeader><TableRow><TableHead>Term</TableHead><TableHead>Course</TableHead><TableHead>Units</TableHead><TableHead>Grade</TableHead><TableHead>Remarks</TableHead></TableRow></TableHeader><TableBody>
          {grades.data?.map((item) => <TableRow key={item.id}><TableCell>{item.schoolYear} · {item.semesterName.replaceAll("_", " ")}</TableCell><TableCell><p className="font-medium">{item.courseCode}</p><p className="text-xs text-slate-500">{item.courseTitle}</p></TableCell><TableCell>{item.units}</TableCell><TableCell className="font-semibold text-[#0f7d82]">{item.grade}</TableCell><TableCell>{item.remarks}</TableCell></TableRow>)}
        </TableBody></Table></div></TabsContent>

        <TabsContent value="credits"><div className="mt-5 grid gap-5 lg:grid-cols-2">
          <section className="overflow-hidden rounded-xl border"><header className="border-b p-4"><h2 className="font-semibold">Posted course credits</h2><p className="text-sm text-slate-500">Credits satisfy curriculum and prerequisite rules without affecting institutional GPA.</p></header><div className="divide-y">{credits.data?.map((credit) => <article key={credit.id} className="p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-medium">{credit.courseCode} · {credit.courseTitle}</p><p className="mt-1 text-sm text-slate-500">{credit.sourceLabel}</p></div><Badge variant={credit.active === false ? "outline" : "default"}>{credit.active === false ? "REVERSED" : `${credit.creditedUnits} UNITS`}</Badge></div></article>)}{!credits.isLoading && !credits.data?.length ? <Empty text="No course credits have been posted." /> : null}</div></section>
          <section className="overflow-hidden rounded-xl border"><header className="border-b p-4"><h2 className="font-semibold">Evaluation cases</h2><p className="text-sm text-slate-500">Track transfer, shifting, second-degree, and migration reviews.</p></header><div className="divide-y">{evaluations.data?.map((evaluation) => <article key={evaluation.id} className="p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-medium">{evaluation.evaluationType.replaceAll("_", " ")}</p><p className="mt-1 text-sm text-slate-500">Target: {evaluation.targetCurriculumCode} · {evaluation.sourceCourseCount} source courses</p></div><Badge variant={statusVariant(evaluation.status)}>{evaluation.status.replaceAll("_", " ")}</Badge></div></article>)}{!evaluations.isLoading && !evaluations.data?.length ? <Empty text="No academic evaluations are on file." /> : null}</div></section>
        </div></TabsContent>

        <TabsContent value="audit"><div className="mt-5 overflow-hidden rounded-xl border"><div className="divide-y">{audits.data?.map((audit) => <article key={audit.id} className="grid gap-4 p-5 sm:grid-cols-[1fr_auto] sm:items-center"><div className="flex gap-3"><FileSearch className="mt-1 size-5 text-[#0f7d82]" /><div><p className="font-medium">Academic graduation audit</p><p className="mt-1 text-sm text-slate-500">{new Date(audit.auditedAt).toLocaleString()} · {audit.earnedUnits} of {audit.totalRequiredUnits} required units</p><p className="mt-1 text-sm text-slate-500">{audit.missingRequiredCount} missing required · {audit.unmetElectiveGroupCount} unmet elective groups · {audit.pendingEvaluationCount} pending</p></div></div><Badge variant={statusVariant(audit.result)}>{audit.result.replaceAll("_", " ")}</Badge></article>)}{!audits.isLoading && !audits.data?.length ? <div className="p-10 text-center"><ShieldCheck className="mx-auto size-8 text-slate-300" /><p className="mt-3 text-sm text-slate-500">No graduation audit has been run yet.</p></div> : null}</div></div></TabsContent>

        <TabsContent value="attendance"><div className="mt-5 overflow-hidden rounded-xl border"><Table><TableHeader><TableRow><TableHead>Course</TableHead><TableHead>Meetings</TableHead><TableHead>Present</TableHead><TableHead>Late</TableHead><TableHead>Absent</TableHead><TableHead>Excused</TableHead></TableRow></TableHeader><TableBody>
...
function Metric({ label, value, detail }: { label: string; value: number; detail: string }) { return <section className="rounded-xl border p-5"><p className="text-sm text-slate-500">{label}</p><p className="mt-2 text-3xl font-semibold text-[#0f7d82]">{value}</p><p className="mt-1 text-xs text-slate-500">{detail}</p></section> }
function Empty({ text }: { text: string }) { return <p className="p-8 text-center text-sm text-slate-500"><Clock3 className="mx-auto mb-2 size-5 text-slate-300" />{text}</p> }

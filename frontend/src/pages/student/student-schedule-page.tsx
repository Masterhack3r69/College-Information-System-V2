...
export default function StudentSchedulePage(){
  ...
  return <main className="mx-auto flex max-w-[1100px] flex-col gap-6 p-4 sm:p-6 lg:p-8">
    <header className="flex flex-wrap items-end justify-between gap-4"><div><h1 className="flex items-center gap-3 text-2xl font-semibold tracking-tight text-foreground sm:text-[1.75rem]"><CalendarDays/>Class Schedule</h1><p className="mt-2 text-muted-foreground">Your confirmed classes, meeting locations, and instructors.</p></div><Select value={selected?`${selected.schoolYearId}|${selected.semesterId}`:""} onValueChange={setChosen}><SelectTrigger className="w-64"><SelectValue placeholder="Select enrolled term"/></SelectTrigger><SelectContent>{terms.data?.map(term=><SelectItem key={`${term.schoolYearId}-${term.semesterId}`} value={`${term.schoolYearId}|${term.semesterId}`}>{term.schoolYear} · {term.semesterName.replaceAll("_"," ")}{term.active?" · Active":""}</SelectItem>)}</SelectContent></Select></header>
    ...
    <div className="flex flex-col gap-5">{days.map(day=>{const rows=schedule.data?.filter(item=>item.dayOfWeek===day)??[];return rows.length?<section key={day} className="overflow-hidden rounded-lg border"><h2 className="border-b bg-surface px-5 py-3 font-semibold">{title(day)}</h2>{rows.map(item=><article key={`${item.scheduleId}-${item.startTime}`} className="grid gap-3 border-b p-5 last:border-0 sm:grid-cols-[130px_1fr_220px]"><p className="font-semibold text-primary">{item.startTime.slice(0,5)}–{item.endTime.slice(0,5)}</p><div><p className="font-semibold">{item.courseCode} · {item.courseTitle}</p><p className="text-sm text-muted-foreground">{item.sectionCode} · {item.faculty} · {item.componentType}</p></div><p className="flex items-center gap-2 text-sm"><MapPin/>{item.deliveryMode==="ONLINE"?item.locationDetails||"Online":item.roomCode||"Room pending"}</p></article>)}</section>:null})}</div>
  </main>
}

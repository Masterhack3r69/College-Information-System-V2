  ...
  return (
    <div className="mx-auto max-w-[1180px] p-4 sm:p-6 lg:p-8">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          <h1 className="flex items-center gap-3 text-2xl font-semibold tracking-tight text-foreground sm:text-[1.75rem]"><GraduationCap />Enrollment</h1>
          <p className="mt-2 text-muted-foreground">Build a valid load from normal-term courses, back subjects, deficiencies, and electives.</p>
        </div>
      ...
      <section className="mt-7 rounded-lg border bg-surface p-5">
        <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            ...
            {current ? <p className="mt-1 text-sm text-muted-foreground">{current.schoolYear} · {current.semesterName.replaceAll("_", " ")} · {current.totalCreditUnits ?? 0} units</p> : null}
          </div>
          {!current ? (
      ...
      {blockers.length ? (
        ...
        <Alert className="mt-5 border-success-foreground/20 bg-success text-success-foreground"><CheckCircle2 /><AlertTitle>Selected load is valid</AlertTitle><AlertDescription>It is ready to submit for Registrar review.</AlertDescription></Alert>
      ) : null}

      <section className="mt-6 overflow-hidden rounded-lg border">
        <header className="flex items-center justify-between border-b px-5 py-4">
          <div><h2 className="font-semibold">Selected load</h2><p className="text-sm text-muted-foreground">{selected.length} course{selected.length === 1 ? "" : "s"}</p></div>
        ...
        {selected.length ? <div className="divide-y">{selected.map((subject) => (
          <article key={subject.id} className="grid gap-3 p-5 sm:grid-cols-[1fr_auto] sm:items-center">
            <div><p className="font-semibold">{subject.courseCode} · {subject.courseTitle}</p><p className="mt-1 text-sm text-muted-foreground"><Clock3 className="mr-1 inline size-3.5" />{meetingsLabel(subject.meetings)} · {subject.sectionCode} · {subject.roomCode ?? "TBA"}</p></div>
        ...
        ))}</div> : <p className="p-8 text-center text-sm text-muted-foreground">Start a draft and choose courses below.</p>}
      </section>

      <section className="mt-6 overflow-hidden rounded-lg border">
        <header className="border-b px-5 py-4"><h2 className="font-semibold">Available classes</h2><p className="text-sm text-muted-foreground">Course rules, seat availability, and schedule conflicts are checked when you add and submit.</p></header>
        <div className="divide-y">
          {classes.data?.map((item) => (
            <article key={item.scheduleId} className="grid gap-4 p-5 lg:grid-cols-[minmax(0,1fr)_230px_145px] lg:items-center">
              <div>
                ...
                <p className="mt-1 text-sm text-muted-foreground">Year {item.curriculumYearLevel} · {item.requiredStatus} · {item.units} units</p>
                <p className="mt-2 text-sm"><Clock3 className="mr-1 inline size-3.5 text-muted-foreground" />{meetingsLabel(item.meetings)}</p>
              </div>
              <div className="text-sm"><p>{item.sectionCode} · {item.roomCode ?? "TBA"}</p><p className="mt-1 text-muted-foreground">{item.faculty || "Faculty TBA"}</p><p className="mt-1"><Users className="mr-1 inline size-3.5 text-muted-foreground" />{Math.max(0, Number(item.availableSeats))} seats available</p></div>
          ...
          {!classes.isLoading && !classes.data?.length ? <p className="p-8 text-center text-sm text-muted-foreground">No unsatisfied curriculum courses are currently offered.</p> : null}
        </div>
      </section>
    </div>
  )

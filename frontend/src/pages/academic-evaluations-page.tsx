...
export default function AcademicEvaluationsPage() {
  ...
  return <div className="app-page">
    <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><h1 className="flex items-center gap-3 text-2xl font-semibold tracking-tight text-foreground sm:text-[1.75rem]"><BookCopy />Academic evaluations</h1><p className="mt-2 text-muted-foreground">Review prior study, post auditable credits, and control explicit curriculum migrations.</p></div>{canApprove ? <NewEvaluationDialog onCreated={setSelectedId} /> : null}</div>
    ...
    <div className="mt-5 grid min-h-[620px] overflow-hidden rounded-lg border lg:grid-cols-[380px_1fr]">
      <aside className="border-b lg:border-b-0 lg:border-r"><header className="border-b p-4 text-sm font-medium text-muted-foreground">{list.data?.length ?? 0} evaluation case{list.data?.length === 1 ? "" : "s"}</header><div className="max-h-[680px] overflow-y-auto divide-y">{list.data?.map((item) => <button key={item.id} type="button" onClick={() => setSelectedId(item.id)} className={`w-full p-4 text-left transition hover:bg-surface ${selectedId === item.id ? "bg-surface ring-1 ring-inset ring-primary" : ""}`}><div className="flex items-start justify-between gap-3"><div><p className="font-medium">{item.studentName}</p><p className="text-xs text-muted-foreground">{item.studentNumber} · {item.programCode}</p></div><Badge variant={badgeVariant(item.status)}>{item.status.replaceAll("_", " ")}</Badge></div><p className="mt-3 text-sm">{item.evaluationType.replaceAll("_", " ")} → {item.targetCurriculumCode}</p></button>)}{!list.isLoading && !list.data?.length ? <p className="p-8 text-center text-sm text-muted-foreground">No cases match this queue.</p> : null}</div></aside>
      <main className="min-w-0">{detail.isLoading ? <div className="grid h-full place-items-center"><Loader2 className="animate-spin text-muted-foreground" /></div> : detail.data ? <CaseWorkspace evaluation={detail.data} canApprove={canApprove} canReview={canReview} /> : <div className="grid h-full place-items-center p-8 text-center"><div><FileClock className="mx-auto size-10 text-muted-foreground/60" /><p className="mt-3 font-medium">Select an evaluation case</p><p className="mt-1 text-sm text-muted-foreground">Its evidence, recommendations, impact, and history will appear here.</p></div></div>}</main>
    </div>
  </div>
}
...
function CaseWorkspace({ evaluation, canApprove, canReview }: { evaluation: EvaluationCase; canApprove: boolean; canReview: boolean }) {
  return <div className="max-h-[740px] overflow-y-auto p-5 md:p-7"><div className="flex flex-col justify-between gap-3 sm:flex-row"><div><div className="flex flex-wrap items-center gap-2"><h2 className="text-xl font-semibold">{evaluation.studentName}</h2><Badge variant={badgeVariant(evaluation.status)}>{evaluation.status.replaceAll("_", " ")}</Badge></div><p className="mt-1 text-sm text-muted-foreground">{evaluation.studentNumber} · {evaluation.evaluationType.replaceAll("_", " ")} · Target {evaluation.targetCurriculumCode}</p></div></div>
    ...
    <section className="mt-6 rounded-lg border"><header className="border-b p-4"><h3 className="font-semibold">Source courses and evidence</h3></header><div className="divide-y">{evaluation.sourceCourses.map((source) => <article key={source.id} className="flex items-start justify-between gap-3 p-4"><div><p className="font-medium">{source.courseCode} · {source.courseTitle}</p><p className="mt-1 text-sm text-muted-foreground">{source.sourceType.replaceAll("_", " ")} · {source.creditUnits} units{source.sourceGrade ? ` · Grade ${source.sourceGrade}` : ""}</p></div>{evaluation.matches.some((match) => match.sourceCourseIds.includes(source.id) && match.status === "RECOMMENDED") ? <Badge>Mapped</Badge> : <Badge variant="outline">Unmapped</Badge>}</article>)}{!evaluation.sourceCourses.length ? <p className="p-6 text-center text-sm text-muted-foreground">No source courses added.</p> : null}</div></section>
    ...
    <section className="mt-6 rounded-lg border"><header className="border-b p-4"><h3 className="font-semibold">Decision history</h3></header><div className="divide-y">{evaluation.history.map((entry) => <article key={entry.id} className="grid gap-1 p-4 sm:grid-cols-[190px_1fr]"><p className="text-sm text-muted-foreground">{new Date(entry.changedAt).toLocaleString()}</p><div><p className="font-medium">{entry.toStatus.replaceAll("_", " ")}</p><p className="text-sm text-muted-foreground">{entry.changedBy}{entry.remarks ? ` · ${entry.remarks}` : ""}</p></div></article>)}</div></section>
  </div>
}
...
function DocumentLinks({ evaluation }: { evaluation: EvaluationCase }) {
  ...
  return <div className="flex flex-wrap items-center gap-2"><span className="text-sm text-muted-foreground">Evidence:</span>{evaluation.documents.map((document) => <Badge key={document.id} variant="outline">{document.documentType}</Badge>)}{unlinked.length ? <select className="h-9 rounded-md border bg-background px-3 text-sm" defaultValue="" onChange={(event) => { if (event.target.value) void link.mutateAsync({ id: evaluation.id, documentId: event.target.value }).then(() => toast.success("Document linked")) }}><option value="">Link student document…</option>{unlinked.map((document) => <option key={document.id} value={document.id}>{document.documentType} · {document.fileName}</option>)}</select> : null}</div>
}
...
function AcademicReview({ evaluation }: { evaluation: EvaluationCase }) {
  ...
  return <section className="mt-6 rounded-lg border border-info-foreground/20 bg-info/60 p-4"><h3 className="font-semibold">Academic equivalency review</h3><div className="mt-4 grid gap-3 sm:grid-cols-2"><Field label="Target curriculum course"><select className="h-9 w-full rounded-md border bg-background px-3 text-sm" value={targetCourseId} onChange={(event) => setTargetCourseId(event.target.value)}><option value="">Select target course</option>{curriculum.data?.courses.map((course) => <option key={course.courseId} value={course.courseId}>{course.courseCode} · {course.courseTitle}</option>)}</select></Field><Field label="Decision"><select className="h-9 w-full rounded-md border bg-background px-3 text-sm" value={decision} onChange={(event) => setDecision(event.target.value as typeof decision)}><option>RECOMMENDED</option><option>REJECTED</option></select></Field></div><div className="mt-4"><Label>Grouped source courses</Label><div className="mt-2 grid gap-2 sm:grid-cols-2">{evaluation.sourceCourses.map((source) => <label key={source.id} className="flex items-start gap-2 rounded-md border bg-background p-3 text-sm"><Checkbox checked={sourceIds.includes(source.id)} onCheckedChange={(checked) => setSourceIds(checked ? [...sourceIds, source.id] : sourceIds.filter((id) => id !== source.id))} /><span><span className="font-medium">{source.courseCode}</span><span className="block text-muted-foreground">{source.courseTitle}</span></span></label>)}</div></div><div className="mt-4 grid gap-3 sm:grid-cols-[140px_1fr_auto] sm:items-end"><Field label="Credited units"><Input type="number" min="0" step="0.5" value={units ?? ""} onChange={(event) => setUnits(event.target.value ? Number(event.target.value) : undefined)} /></Field><Field label="Academic rationale"><Textarea className="min-h-9" value={rationale} onChange={(event) => setRationale(event.target.value)} /></Field><Button onClick={() => void review()} disabled={!targetCourseId || !sourceIds.length || !rationale || save.isPending}>{save.isPending ? <Loader2 className="animate-spin" /> : <CheckCircle2 />}Save decision</Button></div></section>
}
...
function Metric({ label, value }: { label: string; value: number }) { return <div className="rounded-lg border p-3"><p className="text-xs text-muted-foreground">{label}</p><p className="mt-1 text-2xl font-semibold text-primary">{value}</p></div> }
